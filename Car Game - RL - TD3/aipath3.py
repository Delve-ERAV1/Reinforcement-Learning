...
class Actor(nn.Module):
  ...
  def forward(self, x):
    ...
    return  5 * torch.tanh(self.layer_3(x))
...
class Critic(nn.Module):
  ...
  def __init__(self, state_dim, action_dim):
    ...
    self.layer_4 = nn.Linear(state_dim + action_dim, p)
    self.layer_5 = nn.Linear(p, p//2)
    self.layer_6 = nn.Linear(p//2, 1)
...
class TD3(object):
  ...
  def __init__(self, state_dim, action_dim, max_action, batch_size=200, discount=0.99, 
               tau=0.005, policy_noise=0.2, noise_clip=0.5, expl_noise=0, 
  ...
  def update(self, reward, new_signal):
    ...
    if self.timesteps['total_timesteps'] < self.timesteps['start_timesteps']:
        ...
        if self.params["expl_noise"] != 0:
            ...
            action = (action[0].numpy() + np.random.normal(0, self.params["expl_noise"], size=1)).clip(-5, 5).item()
        else:
