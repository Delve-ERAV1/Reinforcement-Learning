...
def retrieve_vector_field(car_position, weight_dest=1):
...
class Game(Widget):
    ...
    def update(self, dt):
        ...
        v_field = retrieve_vector_field([goal_x, goal_y])
        last_signal = [self.car.signal1, self.car.signal2, self.car.signal3, 
                       orientation, *v_field, int(is_on_road), 
                       road_following_performance, distance_to_goal, sand_density,
                       #self.car.velocity[0], self.car.velocity[1], angle_to_goal, -orientation
                       ]
