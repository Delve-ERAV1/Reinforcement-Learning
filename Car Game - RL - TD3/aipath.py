...
class Critic(nn.Module):
  ...
  def __init__(self, state_dim, action_dim):
    ...
    self.layer_2 = nn.Linear(p, p//2)
    #self.layer_3 = nn.Linear(p//2, p//4)
    self.layer_3 = nn.Linear(p//2, 1)
    # Defining the second Critic neural network
    self.layer_5 = nn.Linear(state_dim + action_dim, p)
    self.layer_6 = nn.Linear(p, p//2)
    #self.layer_7 = nn.Linear(p//2, p//4)
    self.layer_7 = nn.Linear(p//2, 1)
...
class TD3(object):
  ...
  def __init__(self, state_dim, action_dim, max_action, batch_size=100, discount=0.99, 
  ...
  def select_action(self, state):
    ...
    steering = action[0].clamp(*self.max_action[0])
    speed = action[1].clamp(*self.max_action[1])
    return np.array([steering.item(), speed.item()])
  ...
  def train(self):
    ...
    next_action[:, 0] = next_action[:, 0].clamp(*self.max_action[0])
    next_action[:, 1] = next_action[:, 1].clamp(*self.max_action[1])
  ...
  def update(self, reward, new_signal):
    ...
    if self.timesteps['total_timesteps'] > 400:
        print("Total Timesteps: {} Since Eval: {} Reward: {} TS: {}".format(self.timesteps['total_timesteps'], 
    ...
    if self.timesteps['total_timesteps'] < self.timesteps['start_timesteps']:
        ...
        action = np.array([
           round(random.uniform(*self.max_action[0]), 2), 
        ...
        if self.params["expl_noise"] != 0:
            noise = np.random.normal(0, self.params["expl_noise"], size=action.shape)
            action = np.clip(action + noise, [self.max_action[0][0], self.max_action[1][0]], 
                                             [self.max_action[0][1], self.max_action[1][1]])
