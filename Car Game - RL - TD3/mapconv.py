...
def extract_grid_optimized(car_position, grid_size=32, padding_value=1):
    """
    Efficiently extracts a 64x64 grid centered around the car's position from the 'sand' array.

    :param sand: A 2D numpy array representing the environment where 0 is road and 1 is non-road.
    :param car_position: A tuple (x, y) representing the car's position.
    :param grid_size: Size of the grid to be extracted, default is 64x64.
    :param padding_value: Value to be used for padding, default is 1 (non-road).
    :return: A 64x64 grid centered around the car's position.
    """
    y, x = car_position
    half_grid = grid_size // 2
    grid = np.full((grid_size, grid_size), padding_value)  # Initialize grid with non-road

    # Start and end coordinates for extraction from 'sand'
    start_x = max(x - half_grid, 0)
    end_x = min(x + half_grid, sand.shape[1])
    start_y = max(y - half_grid, 0)
    end_y = min(y + half_grid, sand.shape[0])

    # Start and end coordinates for placement in the grid
    grid_start_x = half_grid - min(x, half_grid)
    grid_end_x = grid_start_x + end_x - start_x
    grid_start_y = half_grid - min(y, half_grid)
    grid_end_y = grid_start_y + end_y - start_y

    # Update the grid with road portions from 'sand'
    grid[grid_start_y:grid_end_y, grid_start_x:grid_end_x] = sand[start_y:end_y, start_x:end_x]
    grid = np.invert(grid)
...
# Example usage:
# car_pos = (x_coordinate, y_coordinate)
# grid = extract_grid_optimized(sand, car_pos)
...
class Game(Widget):
    ...
    def update(self, dt):
        ...
        # start = time.time()
        car_grid = extract_grid_optimized([int(self.car.x), int(self.car.y)])
        # end = time.time()
